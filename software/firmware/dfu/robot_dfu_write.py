...
def main():
    ...
    if len(args) < 4:
        ...
        print("e.g., python robot_dfu_write.py COM3 firmware.bin 0\r\n")
        print("Port List : \r\n")
        for port in ports:
            print(port)
        sys.exit(0)
    ...
    # Selecting the port
    setDFUTarget = [0xFF, 0xFD, 0x03, 0x00, 0xF6, int(args[3])]
    setDFUTarget.append(calculateCheckSum(setDFUTarget))
    port.write(setDFUTarget)

    # Switching the robot to DFU mode
    rebootBoard = [0xFF, 0xFD, 0x03, 0x00, 0x01, 1]
    ...
    while(True):
        ...
        if writtenPage == pageNum:
            ...
            setDFUTarget = [0xFF, 0xFD, 0x03, 0x00, 0xF6, 8]
            setDFUTarget.append(calculateCheckSum(setDFUTarget))
            port.write(setDFUTarget)
            break
